...
def format_message(sig):
    ...
    msg = (
        ...
        f"سعر الدخول: {sig['price']:.2f}\n"
        f"وقف الخسارة: {sig['stop_loss']:.2f}\n{tp_lines}\n\n"
        f"RSI: {sig['rsi']:.1f} | EMA20: {sig['ema20']:.2f} | EMA50: {sig['ema50']:.2f}\n"
        f"وقت الفحص: {sig['time']}\n"
        f"وقت الارسال: {now_str}\n\n"
        f"اشارة فنية آلية وليست نصيحة استثمارية. نفذها بسعر السوق فورا."
    )
    return msg
...
def get_last_signal(symbol):
    """Read the log file to find the last signal for this symbol (survives restarts)."""
    if not os.path.exists(LOG_FILE):
        return None
    with open(LOG_FILE, "r", newline="", encoding="utf-8") as f:
        ...
        header = next(reader)
        rows = list(reader)
    symbol_rows = [r for r in rows if r[1] == symbol]
    if not symbol_rows:
        return None
    ...
    while True:
        for symbol, name in SYMBOLS.items():
            try:
                ...
                if sig:
                    last_row = get_last_signal(symbol)
                    should_send = True

                    if last_row:
                        last_time_str, last_direction = last_row[0], last_row[2]
                        last_time = pd.to_datetime(last_time_str)
                        sig_time = pd.to_datetime(sig["time"])
                        minutes_since = (sig_time - last_time).total_seconds() / 60

                        if last_direction == sig["direction"] and minutes_since < COOLDOWN_MINUTES:
                            should_send = False
